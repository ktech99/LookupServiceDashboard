...
class App extends Component {
  ...
  hostList(props) {
    ...
    for(var i in hosts){
        ...
        for(var j in host){
          console.log(host[j]);
         var serviceHost = (host[j]["service-host"]).toString();
        //  serviceHost = serviceHost.replace("lookup/host/", "");
         console.log(serviceHost)
         fetch('http://localhost:8080/host?serviceHost='+serviceHost, { headers: { 'Access-Control-Allow-Origin': "http://127.0.0.1:3000" } })
        .then(function(response){
          return response.json();
        })
        .then(function(data) {
